...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      const matchesWeb = !filters.webáruház || item.preferalt_webaruhaz === filters.webáruház;
  ...
  const stats: KPIStats = useMemo(() => {
    return {
      popularStore: getMostFrequent(filteredData.map(d => d.preferalt_webaruhaz)),
  ...
  const filterOptions = useMemo(() => {
    return {
      ...
      webáruház: Array.from(new Set(data.map(d => d.preferalt_webaruhaz))).sort(),
    ...
    <div className="min-h-screen bg-bg-deep text-text-primary font-sans pb-20">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-8 space-y-8">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <KPICard title="Átlagos" value={`${stats.averageReturnRate}%`} label="Visszaküldési arány" icon={<RefreshCcw className="w-4 h-4 text-accent-amber" />} />
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              />
              <InsightCard 
                text={`Visszaküldési arányok: ${stats.averageReturnRate}%.`} 
            ...
            <table className="w-full text-left border-collapse min-w-[1100px]">
              <thead>
                <tr className="bg-bg-deep/50">
                  ...
                  <th className="px-8 py-4 text-[10px] font-bold text-text-dim uppercase tracking-[0.2em] text-center">Visszaküldés</th>
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((row) => (
                    <tr key={row.id} className="hover:bg-bg-card-hover transition-colors group">
                      ...
                      <td className="px-8 py-4 text-sm font-bold text-text-primary">{row.preferalt_webaruhaz}</td>
                      ...
                      <td className="px-8 py-4 text-sm text-center">
                        <span className={`px-2 py-1 rounded-full text-xs font-black ${row.visszakuldesi_arany_szazalek > 15 ? 'text-red-400 bg-red-400/10' : 'text-accent-teal bg-accent-teal/10'}`}>
                          {row.visszakuldesi_arany_szazalek}%
                        </span>
                      </td>
...
function AgeGenderStoreChart({ data, filters }: { data: PurchaseRecord[], filters: FilterOptions }) {
  ...
  if (filters.korcsoport && filters.nem) {
    ...
    const storeCounts = stores.map(store => groupData.filter(d => d.preferalt_webaruhaz === store).length);
  ...
  const chartData = {
    ...
    datasets: stores.map((store, i) => ({
      ...
      data: activeGroups.map(label => {
        ...
        const count = groupData.filter(d => d.preferalt_webaruhaz === store).length;
...
function BenchmarkBarChart({ items, benchmarkAverage }: { items: { label: string, value: number, count: number }[], benchmarkAverage: number }) {
  const chartMax = Math.max(
    5,
    Math.ceil((Math.max(...items.map(item => item.value), benchmarkAverage) + 5) / 5) * 5
  );
    ...
    <div className="h-full w-full flex flex-col justify-between py-2 relative">
      ...
      <div className="flex-1 flex flex-col justify-around gap-2">
        {items.map((item, idx) => {
          ...
          const diff = Number((item.value - benchmarkAverage).toFixed(1));
...
function StoreReturnRateChart({ data }: { data: PurchaseRecord[] }) {
  ...
  const storeAverages = stores.map(store => {
    const storeRecords = data.filter(d => d.preferalt_webaruhaz === store);
    return {
      label: store,
  ...
  console.table(storeAverages);
  console.log("Store benchmark average:", benchmarkAverage);
  console.log("Flipkart:", storeAverages.find(item => item.label === "Flipkart"));

  return (
    <div className="h-full">
      <BenchmarkBarChart items={storeAverages} benchmarkAverage={benchmarkAverage} />
    </div>
  );
}
...
function ReturnRateAnalysisChart({ data, dimension }: { data: PurchaseRecord[], dimension: string }) {
  ...
  if (dimension === "Nem × Életkor") {
    ...
    console.log("0-18 / Nő:", items.find(item => item.label === "0-18 / Nő"));
    console.log("19-30 / Férfi:", items.find(item => item.label === "19-30 / Férfi"));
    console.log("19-30 / Nő:", items.find(item => item.label === "19-30 / Nő"));
    console.log("31-50 / Nő:", items.find(item => item.label === "31-50 / Nő"));
    ...
    const stores = ["Amazon", "Flipkart", "Myntra"];
    items = stores.map(s => {
      const records = data.filter(d => d.preferalt_webaruhaz === s);
      return {
        label: s,
        value: averageReturnRate(records),
        count: records.length
